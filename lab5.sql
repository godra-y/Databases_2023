...
where book_ID not in (select distinct book_ID from borrowed_books);

select title from Books
where author_id in (select author_id from Authors where extract(year from birth_date)<1930);

insert into Authors (author_id, first_name, last_name, birth_date)
values
    (21, 'Haruki', 'Murakami', '1949-12-01'),
    (22, 'Nassim', 'Taleb', '1960-11-09'),
...
select A.author_id, A.first_name, A.last_name, (select count(*) from Books B where B.author_id = A.author_id) as book_count
from Authors A
...
where date_part('month', return_date)>4;
...
--2
select A.first_name, A.last_name, (select count(*) from Books B where B.author_id=A.author_id) as books_published
from Authors A
order by books_published desc
limit 15;

--3
select B.title, count(*) as totalborrows
from Books as B
