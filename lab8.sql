...
where price < 100 and weight < 10;

--4
insert into cheap_and_light_products(id, name, department, price, weight)
...
select o.id as order_id, u.first_name, u.last_name
from orders o join users u on o.user_id = u.id;

--7
select ou.first_name || ' ' || ou.last_name as user_name, o.order_date
from order_user ou join orders o on ou.order_id = o.id
where o.shipment_date is null;

--8
select ou.first_name || ' ' || ou.last_name as user_name
from order_user ou
join orders on ou.order_id = orders.id
join products on orders.product_id = products.id
where products.department = 'Tools' and paid = true;

--9
select ou.first_name || ' ' || ou.last_name as user_name
from order_user ou join orders ON ou.order_id = orders.id
...
--13
drop view cheap_and_light_products, order_user, user_sum;


--1
create view dep_max as
select department, max(price) as max_price
from products
group by department;

--2
select id, name, products.department, price, weight
from products
join dep_max dm on products.department = dm.department
where products.price = max_price;

--3
/*
 • The defining query of the view must has exactly one entry in
the FROM clause, which can be a table or another updatable
view.
• The defining query must not contain one of the following
clauses at top level: GROUP BY, HAVING, LIMIT,
OFFSET, DISTINCT, WITH, UNION, INTERSECT, and
EXCEPT.
• The selection list must not contain any window function
or set-returning function or any aggregate function such
as SUM, COUNT, AVG, MIN, MAX, etc.
 */
